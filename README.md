...
`conversations.jsonl` — главный машинный артефакт: одна строка JSON = одна
полная переписка с контекстом объявления. Каждый conversation самодостаточен:
внутри есть `chat_id`, `item_id`, доступные поля объявления, период, счётчики и
все сообщения по порядку. `items.json` остаётся общим справочником объявлений.
`conversations_index.csv` помогает быстро фильтровать корпус по ID, объявлению,
периоду и количеству сообщений. `human_readable/*.txt` хранит те же переписки в
читаемом виде, который удобно загружать в ChatGPT.

Сообщения собираются из всех сохранённых RAW-страниц `raw/messages/`, одинаковые
`message_id` не дублируются, порядок выставляется по timestamp. Направления
показываются как «Клиент», «Мы» или «Неизвестный участник». Для нетекстовых
сообщений используются плейсхолдеры вроде `[изображение]`, `[голосовое]`,
`[системное сообщение]`; бинарные файлы в TXT не вставляются.
