...
`chats_index.csv` и `chats_index.json` содержат общий список чатов: безопасный
локальный ID, исходный `chat_id`, `item_id`, заголовок объявления, тип чата,
первое и последнее сообщение, количество сообщений и ссылки на JSON/TXT файла
чата. `chats/*.json` хранит один чат в структурированном виде для программного
анализа. `chats/*.txt` хранит тот же чат в человекочитаемом формате, который
удобно загружать в ChatGPT.
