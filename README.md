...
актуальному на 6 сентября 2026 года:
...
Метод `items` предоставляет группировки `day`, `week`, `month`, `totals`,
пагинацию `limit/offset` с `limit <= 1000`, лимит 100 запросов в минуту и
историю до 270 дней. Экспортёр делает однодневные запросы с `grouping=totals`,
...
100 запросов в минуту и историю до 510 дней. Экспортёр запрашивает `["all"]`
...
Между запросами статистики выдерживается интервал 0,61 секунды. HTTP 429 учитывает
...
Документированные горизонты 270/510 дней означают всю доступную API историю и
...
Полный живой экспорт в рамках разработки v0.3 не запускался. Синтетические тесты
проверяют контракт, пагинацию, ошибки и recovery; права конкретного аккаунта можно
подтвердить только пользовательским запуском.

## Ограничения Avito API

- Статистика объявлений старше 270 дней и расходы старше 510 дней API не отдаёт.
